...
/* -------------------------------------------------------------------------
 * VERIFICATION PASS, 2026-07-18 (static-dump re-query, no live Ghidra
 * bridge): re-confirmed omap_spi_write's body byte-for-byte against a fresh
 * decompile - every branch, both busy-wait polarities, and the final
 * read-modify-write all match exactly, no corrections needed. Also
 * re-confirmed the 8-caller list above (3x cad_calibration_sweep, 1x
 * cad_trigger_calibration, 1x cad_init, 1x cpsoc_analog_poll_channel, 2x
 * cpsoc_analog_poll_task) against a fresh xref query - unchanged. Two
 * DAT_ constants newly resolved:
 *  - DAT_c0003668 (the fault call's "file" argument) = 0xc0022d44, exactly
 *    the "../MCU/Component/OmapL108Spi.cpp" string address - confirms both
 *    hard-fault call sites genuinely are this file's own 2 anchor xrefs.
 *  - DAT_c0003664 (omap_spi_retry_delay's own argument) = 0xc0e00068 - well
 *    outside this image's ~0xc0000000-0xc0300000 code/data range, so not a
 *    pointer into anything this project has otherwise mapped; plausibly a
 *    real MMIO peripheral/timer register elsewhere in the SoC's address
 *    space, not confirmed.
 *
 ...
 *    pass on cad.c, out of scope for this SPI-driver-focused file (NOTE:
 *    cad.c's own 2026-07-17 calibration-engine pass has since reconstructed
 *    this as cad_calibration_sweep - this file's own cross-reference to it
 *    by FUN_ name only is now stale, though harmless).
 * ------------------------------------------------------------------------- */
