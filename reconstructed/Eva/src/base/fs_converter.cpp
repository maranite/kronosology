 ...
 * recovery. `SetFilterCoeffs(int,int,float,int,int)` bodies below (both the
 * literal 1-byte "return;" base stub and CFsCwInterpolation's shift/un-shift-
 * preserving override) and both `Process()` stubs are intentionally minimal --
 * see the header comment for exactly which real ground-truth bytes each one
 * corresponds to and why the deeper logic is out of scope this pass.
 */
...
void CFsConverterNormal::BuildFilterCoeffTable(int, int, int, int, int)
{
	/* DEFERRED -- see header comment. Real ground truth allocates
	 * mState->mPhaseCoeffs[0..maxPhases] and fills each with
	 * mFilterCoeffs.GetFilterCoeff() samples; not transcribed this pass. */
}
...
{
	/* Real override un-shifts mOversamplingRate (stored left-shifted by 14
	 * bits in this class), calls the base's (deferred) coefficient-table
	 * builder with (a,b,d,e,1), then re-shifts. Shift/un-shift preserved
	 * for internal consistency even though BuildFilterCoeffTable() itself
	 * is a stub -- see header comment. Fixed-point scale-factor tail
	 * (real ground truth: `param_2*mOversamplingRate/param_3` doubled 14
	 * times then truncated to mState->mDecimationFactor) DEFERRED alongside
	 * BuildFilterCoeffTable() since it depends on the same not-yet-modeled
	 * coefficient state. */
	mState->mOversamplingRate >>= 0xe;
