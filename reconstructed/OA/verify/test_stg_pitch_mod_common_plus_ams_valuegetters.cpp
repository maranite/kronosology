...
/*
 * test_stg_pitch_mod_common_plus_ams_valuegetters.cpp  -  KAT for
 * CSTGPitchModCommonPlusAMS's Get* family -- both real weak-symbol
 * ctx-only candidates, see
 * ../src/engine/stg_pitch_mod_common_plus_ams_valuegetters.cpp.
 *
...
int main(void)
{
	printf("CSTGPitchModCommonPlusAMS value-getter family known-answer test (2 methods)\n");
	printf("=============================================================================\n");
	for (unsigned int i = 0; i < BUFSZ; i++)
		g_buf[i] = (unsigned char)(i*0x9f + 0x37);
	*(int *)(g_ctxbuf + 0x4) = 3;

	CSTGPitchModCommonPlusAMS *s = (CSTGPitchModCommonPlusAMS *)g_buf;
	CSTGPatchMessageContext &ctx = *(CSTGPatchMessageContext *)g_ctxbuf;

	s->GetAMSSource(ctx);
	check_eq("CSTGPitchModCommonPlusAMS::GetAMSSource value", CSTGParamsOwner::sValueGetterTemp.value, -1L);
	s->GetAMSIntensity(ctx);
	check_eq("CSTGPitchModCommonPlusAMS::GetAMSIntensity value", CSTGParamsOwner::sValueGetterTemp.value, 1623270019L);
	check_eq("CSTGPitchModCommonPlusAMS::GetAMSIntensity displayValue", CSTGParamsOwner::sValueGetterTemp.displayValue, 1623270019L);
