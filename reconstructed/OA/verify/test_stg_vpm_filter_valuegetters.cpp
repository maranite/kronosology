...
/*
 * test_stg_vpm_filter_valuegetters.cpp  -  KAT for CSTGVPMFilter's Get*
 * family -- all 3 real weak-symbol ctx-only candidates, see
 * ../src/engine/stg_vpm_filter_valuegetters.cpp.
 *
...
int main(void)
{
	printf("CSTGVPMFilter value-getter family known-answer test (3 methods)\n");
	printf("=================================================================\n");
	for (unsigned int i = 0; i < BUFSZ; i++)
		g_buf[i] = (unsigned char)(i*0x9f + 0x37);
	*(int *)(g_ctxbuf + 0x4) = 3;

	CSTGVPMFilter *s = (CSTGVPMFilter *)g_buf;
	CSTGPatchMessageContext &ctx = *(CSTGPatchMessageContext *)g_ctxbuf;

	s->GetRoutingValue(ctx);
	check_eq("CSTGVPMFilter::GetRoutingValue value", CSTGParamsOwner::sValueGetterTemp.value, 0L);
	s->GetFilterAOutputLevelAMSMode(ctx);
	check_eq("CSTGVPMFilter::GetFilterAOutputLevelAMSMode value", CSTGParamsOwner::sValueGetterTemp.value, -92L);
	s->GetFilterBOutputLevelAMSMode(ctx);
	check_eq("CSTGVPMFilter::GetFilterBOutputLevelAMSMode value", CSTGParamsOwner::sValueGetterTemp.value, -31L);
