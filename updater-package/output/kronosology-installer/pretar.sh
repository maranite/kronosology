...
case "${1:-}" in
    --verify)   MODE="verify" ;;
    --revert)   MODE="revert" ;;
    -h|--help)  echo "usage: $0 [--verify | --revert]"; exit 0 ;;
    # Any other arg (or none) → apply mode.
    # We deliberately don't reject unknown args because UpdateOS invokes the
    # script as a PRETAR with arguments we don't control (varies by Korg OS
    # version) and we need to accept whatever it passes.
    *)          MODE="apply" ;;
esac
