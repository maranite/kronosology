...
**`rtwrap_*` is not provided by `loadmod.ko`.** `loadmod.ko` has zero `rtwrap_*`/`stg_*`
symbols (confirmed via `nm`). `OA.ko`'s own `rtwrap_*` functions (e.g.
`rtwrap_set_runnable_on_cpuid` @ `0x118ff0`) are defined, real code inside `OA.ko`
itself — the genuine RTAI primitives (`rt_task_init`/`rt_sem_wait`) are the actual
undefined/imported externs, resolved against the real `rtai_*.ko` stack. Each
STG-family module (`OA.ko`, `OmapNKS4Module.ko`, …) carries its own private,
statically-linked `rtwrap_*` veneer.
...
- [../modules/loadmod.ko.md](../modules/loadmod.ko.md) — boot-integrity module (not the
  `rtwrap_*` provider — see note above)
